...
DB_PATH = os.environ.get("DB_PATH", "novaai.db")
...
def ensure_user(message: Message):
    ...
    conn.commit()
    row = conn.execute("SELECT messages_left FROM users WHERE user_id=?", (u.id,)).fetchone()
    conn.close()
    return row[0]
...
def balance(user_id: int):
    conn = db()
    row = conn.execute("SELECT messages_left FROM users WHERE user_id=?", (user_id,)).fetchone()
    conn.close()
...
def consume(user_id: int):
    conn = db()
    conn.execute("UPDATE users SET messages_left=messages_left-1 WHERE user_id=? AND messages_left>0", (user_id,))
    conn.commit()
    conn.close()
...
def recent_chat(user_id: int, limit: int = 12):
    conn = db()
    rows = conn.execute("SELECT role,content FROM chats WHERE user_id=? ORDER BY id DESC LIMIT ?", (user_id, limit)).fetchall()
    conn.close()
...
async def ask_ai(user_id: int, text: str) -> str:
    ...
    payload = {
        "model": MODEL,
        "input": input_items,
        "instructions": "أنت Nova AI، مساعد ذكاء اصطناعي عربي مفيد وودود. أجب بالعربية عند استخدام العربية، وباختصار مفيد. لا تدّعي أنك إنسان.",
    }
...
async def chat(message: Message):
    ...
    if balance(message.from_user.id) <= 0:
        await message.answer("❌ انتهى رصيدك المجاني. يمكننا إضافة نظام الباقات والدفع لاحقًا.")
        return
